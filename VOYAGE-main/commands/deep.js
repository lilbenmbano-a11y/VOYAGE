...
module.exports = async (sock, chatId, message) => {
    try {
        ...
        if (!audMsg) return reply(sock, chatId, '❌ Reply to an audio with .deep', message);
        await reply(sock, chatId, '🎚️ Making voice deeper...', message);
        const stream = await downloadContentFromMessage(audMsg, 'audio');
        const chunks = []; for await (const c of stream) chunks.push(c);
        const tmpIn  = path.join('./temp', `deep_in_${Date.now()}.ogg`);
        const tmpOut = path.join('./temp', `deep_out_${Date.now()}.mp3`);
        fs.writeFileSync(tmpIn, Buffer.concat(chunks));
        execSync(`ffmpeg -i "${tmpIn}" -af "asetrate=44100*0.7,aresample=44100" "${tmpOut}" -y`);
