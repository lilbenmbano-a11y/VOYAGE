...
module.exports = async (sock, chatId, message) => {
    try {
        ...
        if (!audMsg) return reply(sock, chatId, '❌ Reply to an audio with .earrape', message);
        await reply(sock, chatId, '🔥 Applying earrape effect... 🔥', message);
        const stream = await downloadContentFromMessage(audMsg, 'audio');
        const chunks = []; for await (const c of stream) chunks.push(c);
        const tmpIn  = path.join('./temp', `er_in_${Date.now()}.ogg`);
        const tmpOut = path.join('./temp', `er_out_${Date.now()}.mp3`);
        fs.writeFileSync(tmpIn, Buffer.concat(chunks));
        execSync(`ffmpeg -i "${tmpIn}" -af "volume=15dB,acrusher=1:1:100:0:log" "${tmpOut}" -y`);
